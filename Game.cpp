...
void Game::initGame(int w, int h, const QString &title, const QIcon &icon)
{
    ...
    progressBar =
        new MyProgressBar((CUBE_LENGTH * MAP_WIDTH - MY_PROGRESS_BAR_WIDTH) / 2, CUBE_LENGTH, 60);
    ...
    pauseMenu = new PauseMenu(this, (MAP_WIDTH * CUBE_LENGTH - PAUSE_MENU_WIDTH) / 2,
                              (MAP_HEIGHT * CUBE_LENGTH - PAUSE_MENU_HEIGHT) / 2);
    ...
    finishMenu = new FinishMenu(this, (MAP_WIDTH * CUBE_LENGTH - PAUSE_MENU_WIDTH) / 2,
                                (MAP_HEIGHT * CUBE_LENGTH - PAUSE_MENU_HEIGHT) / 2);
...
void Game::finishGame()
{
    ...
    if (gameMode == GS_single_mode)
        emit signalFinish(1, scoreBoard1->getScore());
    else if (gameMode == GS_double_mode) {
        if (scoreBoard1->getScore() > scoreBoard2->getScore()) emit signalFinish(2, 1);
        if (scoreBoard1->getScore() < scoreBoard2->getScore()) emit signalFinish(2, 2);
        if (scoreBoard1->getScore() == scoreBoard2->getScore()) emit signalFinish(2, 3);
    }
    state = GS_finish;
}
...
void Game::loadGame()
{
    ...
    if (gameMode == GS_double_mode) scoreBoard2->loadState(filePath);
...
void Game::generateItem()
{
    ...
    QRect inRect(
        MAP_BLOCK_LEFT + (MAP_BLOCK_RIGHT - MAP_BLOCK_LEFT - N * CUBE_LENGTH) / 2 - item->width(),
        MAP_BLOCK_UP + (MAP_BLOCK_DOWN - MAP_BLOCK_UP - M * CUBE_LENGTH) / 2 - item->height(),
        item->width() + N * CUBE_LENGTH, item->height() + M * CUBE_LENGTH);
    while (true) {
...
void Game::boxCollitionDect1()
{
    ...
    if (triggerElapsedTimer1.elapsed() < 600) return;
...
void Game::boxCollitionDect2()
{
    ...
    // if only one collision && trigger elapse>500 , then triggered
    if (triggeredBoxes.size() != 1) return;
    if (triggerElapsedTimer2.elapsed() < 600) return;
...
{
    if (playerID == 1) emit signalScore1(x);
    if (playerID == 2) emit signalScore2(x);
}
...
void Game::shuffleBox()
{
    ...
    if (player1->getTriggeredBox() != nullptr) color1 = player1->getTriggeredBox()->getColor();
    if (player2->getTriggeredBox() != nullptr) color2 = player2->getTriggeredBox()->getColor();
...
void Game::solubleCheck()
{
    ...
    for (auto i : Mgr->getEntity(ET_box)) {
        ...
        // 边缘检测
        if (boxMatrix[r - 1][c] != 0 && boxMatrix[r + 1][c] != 0 && boxMatrix[r][c - 1] != 0 &&
            boxMatrix[r][c + 1] != 0)
            continue;
...
void Game::saveGameState(const QString &filePath)
{
    ...
    if (gameMode == GS_single_mode) fileContent.replace(regex, "gamemode: singleMode");
    if (gameMode == GS_double_mode) fileContent.replace(regex, "gamemode: doubleMode");
...
void Game::loadGameState(const QString &filePath)
{
    ...
    while (!in.atEnd()) {
        ...
        if (line.startsWith("gamemode:")) {
            ...
            if (mode == "doubleMode") gameMode = GS_double_mode;
            if (mode == "singleMode") gameMode = GS_single_mode;
            break;
        }
    }
...
void Game::saveItems(const QString &filePath)
{
    ...
    for (auto i : Mgr->getEntity(ET_item)) {
        item = (Item *)i;
        itemData += QString::number(item->getItemType()) + " " + QString::number(item->getX()) +
...
void Game::loadItems(const QString &filePath)
{
    ...
    for (int i = 0; i < num && !in.atEnd(); ++i) {
        ...
        QStringList numbers = line.split(QRegularExpression("\\s+"));
        Mgr->addEntity(
            new Item((ItemType)numbers[0].toInt(), numbers[1].toInt(), numbers[2].toInt()));
    }
...
bool Game::elimatable(int r1, int c1, int r2, int c2, int showPath)
{
    ...
    return horizonElimatable(r1, c1, r2, c2, showPath) ||
           verticalElimatable(r1, c1, r2, c2, showPath) ||
           oneCornerElimatable(r1, c1, r2, c2, showPath) ||
           twoCornerElimatable(r1, c1, r2, c2, showPath);
}
...
bool Game::horizonElimatable(int r1, int c1, int r2, int c2, int showPath)
{
    ...
    // draw path
    if (showPath) addLine(r1, c1, r2, c2, showPath);
...
bool Game::canReachEdge(const int r, const int c)
{
    ...
    while (!q.empty()) {
        ...
        for (const auto &direction : directions) {
            ...
            if (r >= 1 && r <= M && c >= 1 && c <= N && !visited[newR][newC] &&
                boxMatrix[newR][newC] == 0) {
                q.push({newR, newC});
